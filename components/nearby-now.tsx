...
const categoryMeta: Record<NearbyCategory, { label: string; icon: LucideIcon; tint: string }> = {
  eat: { label: "Nearby Eat", icon: UtensilsCrossed, tint: "bg-orange-100 text-orange-700 dark:bg-orange-500/15 dark:text-orange-200" },
  coffee: { label: "Nearby Coffee", icon: Coffee, tint: "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200" },
  do: { label: "Nearby Do", icon: WandSparkles, tint: "bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-200" }
};
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-foreground">Nearby Now</h1>
        ...
        <button
          className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-bold text-foreground hover:border-primary hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/35 disabled:opacity-60"
          onClick={locate}
          type="button"
          disabled={loading}
        >
      ...
      <section className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-slate-100 via-sky-100 to-cyan-100 p-6 shadow-sm dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_85%_20%,rgba(56,189,248,0.35),transparent_52%)] dark:bg-[radial-gradient(circle_at_85%_20%,rgba(34,211,238,0.16),transparent_52%)]" />
                ...
                <button
                  ...
                  className={
                    active
                      ...
                      : "inline-flex min-h-11 items-center gap-2 rounded-xl border border-border/70 bg-card/90 px-4 py-2 text-sm font-bold text-foreground hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/35"
                  }
            ...
            {(Object.keys(categoryMeta) as NearbyCategory[]).map((key) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setActiveCategory(key)}
                  className="group flex min-h-16 items-center gap-3 rounded-xl border border-border/60 bg-card/90 px-4 py-3 text-left shadow-sm transition-transform hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/35"
                >
                  <span className={`inline-flex h-9 w-9 items-center justify-center rounded-lg ${meta.tint}`}>
                    <Icon className="h-4 w-4" />
                  </span>
                  <span className="text-sm font-bold text-foreground group-hover:text-primary">{meta.label}</span>
                </button>
              );
            })}
          </div>
      ...
      {error ? (
        <div role="alert" aria-live="assertive" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          ...
          <button
            type="button"
            onClick={locate}
            className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-xs font-bold text-red-700 hover:bg-red-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-300 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200 dark:hover:bg-red-500/15"
          >
      ...
      {data?.degraded ? (
        <div
          role="status"
          aria-live="polite"
          className={`rounded-xl border p-4 text-sm ${data.stale ? "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200" : "border-border bg-muted text-foreground"}`}
        >
          <p className="inline-flex items-center gap-2 font-semibold">
            <AlertTriangle className="h-4 w-4" />
      ...
      {data?.mockMode ? (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground" role="status" aria-live="polite">
      ...
      {data ? (
        <section className="space-y-5">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <h2 className="text-2xl font-extrabold tracking-tight text-foreground">Highly Rated Nearby</h2>
          ...
          {activePlaces.length ? (
              ...
              {activePlaces.map((place) => (
                <article key={place.placeId} className="social-card overflow-hidden hover:shadow-md">
                  <div className="relative h-44 overflow-hidden bg-muted">
                    ...
                    <span className="absolute right-3 top-3 rounded-lg bg-card/95 px-2 py-1 text-[11px] font-bold text-foreground">
                  ...
                  <div className="space-y-2 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="line-clamp-1 text-lg font-extrabold tracking-tight text-foreground">{place.name}</h3>
                      <span className="text-xs font-bold text-primary">Nearby</span>
                    </div>
                    <p className="line-clamp-2 text-sm text-muted-foreground">{place.address || "Address unavailable"}</p>
            ...
            <div className="social-empty-panel">
              <p className="font-semibold text-foreground">No places in this category yet.</p>
