...
export function CreateItineraryForm({ initialPlaceId }: { initialPlaceId?: string }) {
  ...
  return (
    <Card className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Create Personalized Itinerary</h1>
        <p className="text-sm text-muted-foreground">Paste social links and caption text to generate and optionally publish.</p>
      </div>

      <div className="space-y-2">
        <label className="text-sm font-medium">Caption / context</label>
        <Textarea value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Weekend in Paris with food and museums" />
      </div>

      <div className="space-y-2">
        <label className="text-sm font-medium">Social links (1-5)</label>
        <Textarea value={linksInput} onChange={(e) => setLinksInput(e.target.value)} placeholder="https://instagram... (one per line)" />
      </div>

      <div className="grid gap-2 md:grid-cols-4">
        <select className="rounded border px-3 py-2 text-sm" value={pref.budget} onChange={(e) => setPref((p) => ({ ...p, budget: e.target.value as PreferenceState["budget"] }))}>
          <option value="budget">Budget</option>
          <option value="mid">Mid</option>
          <option value="luxury">Luxury</option>
        </select>
        <select className="rounded border px-3 py-2 text-sm" value={pref.days} onChange={(e) => setPref((p) => ({ ...p, days: Number(e.target.value) as PreferenceState["days"] }))}>
          <option value={1}>1 day</option>
          <option value={2}>2 days</option>
          <option value={3}>3 days</option>
        </select>
        <select className="rounded border px-3 py-2 text-sm" value={pref.pace} onChange={(e) => setPref((p) => ({ ...p, pace: e.target.value as PreferenceState["pace"] }))}>
      ...
      <div className="grid gap-2 md:grid-cols-2">
        <Input placeholder="Vibe tags (comma separated)" value={pref.vibeTags} onChange={(e) => setPref((p) => ({ ...p, vibeTags: e.target.value }))} />
        <Input placeholder="Dietary prefs (optional)" value={pref.dietary} onChange={(e) => setPref((p) => ({ ...p, dietary: e.target.value }))} />
      </div>

      <div className="flex flex-wrap gap-2">
        <Button variant="outline" onClick={detectLocation}>Parse links + detect location</Button>
        <Button onClick={buildItinerary}>Generate itinerary</Button>
      </div>

      {hints.length ? <p className="text-xs text-muted-foreground">Hints: {hints.join(", ")}</p> : null}
      ...
      {suggestions.length ? (
        <div className="space-y-2">
          ...
          {suggestions.map((suggestion) => (
            <label key={suggestion.placeId} className="flex items-center gap-2 rounded border px-3 py-2 text-sm">
              <input
                type="radio"
                name="location"
      ...
      {tripUrl ? <p className="text-sm">Trip: <a className="underline" href={tripUrl}>{tripUrl}</a></p> : null}
      {postUrl ? <p className="text-sm">Post: <a className="underline" href={postUrl}>{postUrl}</a></p> : null}
